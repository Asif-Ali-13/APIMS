...
type CircuitStateValue = (typeof CircuitState)[keyof typeof CircuitState];
...
/**
 * A simple implementation of the Circuit Breaker pattern in TypeScript. 
 * 
 * Usage: 
 * 
 ...
 * 
 * The circuit breaker will automatically transition between states (CLOSED, OPEN, HALF_OPEN)
 * based on the success and failure of requests, and will log state changes and metrics for 
 * monitoring purposes.
 */
export class CircuitBreaker {
    ...
    private readonly logger: CircuitLogger;

    private _state: CircuitStateValue;
     ...
     * @private
     */
    private _transitionTo(newState: CircuitStateValue): void {
    ...
    /**
     * Opens the circuit, preventing further requests from being allowed until the cooldown
     * period has elapsed. This method is called when the failure threshold is reached or 
     * when a test request in the HALF_OPEN state fails.
     * @private
     */
    ...
    /**
     * Resets the circuit breaker to the CLOSED state, allowing requests to be processed
     * normally. This method is called when a test request in the HALF_OPEN state succeeds
     * or can be called manually for debugging purposes.
     * @private
     */
    private _reset(): void {
        this._state = CircuitState.CLOSED;
    ...
    allowRequest(): boolean {
        ...
        // In OPEN state, no requests are allowed until cooldown has elapsed, 
        // then it transitions to HALF_OPEN.
     ...
     * If the circuit breaker is in the HALF_OPEN state, it counts the success and transitions
     * to CLOSED if the required number of successful attempts is reached. If the circuit
     * breaker is in the CLOSED state and there were previous failures, 
    ...
    /**
     * Records a failed request. If the circuit breaker is in the HALF_OPEN state, 
     * it immediately transitions back to OPEN. If the circuit breaker is in the CLOSED state, 
