 ...
 * This includes common network-related errors and RabbitMQ-specific errors that 
 * indicate transient issues with the connection or channel. The isRetryable function 
 * uses this list to determine if an error should trigger a retry attempt.
 * 
 * @constant {string[]}
 */
...
type RetryableError = Partial<Error> & { code?: string };
...
export function isRetryable(err: unknown): boolean {
    ...
    const retryError = err as RetryableError;
    const msg = (retryError.message || '').toLowerCase()
    const code = (retryError.code || '').toUpperCase();
...
/**
 * Implements a retry strategy with exponential backoff and jitter for handling transient
 * errors when publishing messages to RabbitMQ. The strategy allows for a configurable number
 * of maximum retries, base delay, maximum delay, and jitter factor to randomize the delay 
 * and avoid thundering herd problems. The shouldRetry method determines if another retry
 * attempt should be made based on the current attempt count, while the wait method returns a
 * promise that resolves after the calculated delay for the next retry attempt.
 * 
 * @class RetryStrategy
 * @constructor
...
export class RetryStrategy {
    ...
    constructor(opts: { 
        maxRetries?: number; baseDelayMs?: number; maxDelayMs?: number; jitterFactor?: number 
    } = {}) {
