...
interface RabbitMQLike {
    connect: () => Promise<unknown>;
}

interface RabbitMQWithConnection extends RabbitMQLike {
    connection?: ChannelModel | null;
}

interface Waiter {
    resolve: (value: ConfirmChannel) => void;
...
export class ConfirmChannelManager extends EventEmitter {
    private readonly _rabbitmq: RabbitMQLike;
     ...
     */
    constructor({ rabbitmq, logger }: { rabbitmq: RabbitMQLike; logger?: ChannelLogger }) {
        super();
    ...
    /**
     * Returns a promise that resolves to a RabbitMQ confirm channel. 
     * If the channel is already established, it returns it immediately.
     *  
     * If the channel is in the process of being established, 
     * it waits for that process to complete and then returns the channel. 
     * 
     * If no channel exists and one is not currently being established, 
     * it initiates the creation of a new confirm channel.
     * 
    ...
    /**
     * Establishes a new RabbitMQ confirm channel. 
     * If a channel is already being established, it waits for that process to complete. 
     * 
     * If the channel is successfully established, it resolves any pending promises 
     * waiting for the channel. If an error occurs during channel creation, 
     * it rejects any pending promises and throws the error.
     * 
     * @returns {Promise<any>} A promise that resolves to a RabbitMQ confirm channel.
    ...
    private async _connect(): Promise<ConfirmChannel> {
        ...
        try {
            ...
            let connection: ChannelModel | null | undefined;

            const existingConnection = (this._rabbitmq as RabbitMQWithConnection).connection;
            if (existingConnection) {
                ...
                const connected = (this._rabbitmq as RabbitMQWithConnection).connection;
                if (!connected) {
            ...
            confirmChannel.on("error", (err: Error & { code?: string }) => {
