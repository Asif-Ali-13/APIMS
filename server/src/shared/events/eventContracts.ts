 ...
 * This module centralizes the event type definitions to ensure consistency across the
 * application and to provide a single source of truth for all event types. 
 * Each event type is represented as a string constant that can be used when publishing 
 * events to RabbitMQ or when subscribing to events in different parts of the application.
 * This approach helps to avoid typos and makes it easier to manage event types.
 * 
